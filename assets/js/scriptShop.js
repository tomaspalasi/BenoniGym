...
function addEventListenerList(list) {
    for (var i = 0, len = list.length; i < len; i++){
        list[i].addEventListener("click", (e) =>{
                ...
                compra = [remera, cant, size, price, finalPrice]
                // BUSCAR EN LA VARIABLE cargado la remera, 
                carrito.push(compra)
                localStorage.setItem("carrito", carrito)
                // localStorage.setItem("carrito", cargado)
                // json.setItem("carrito", cargado)
            })
    }
}

addEventListenerList(btnCompra, carrito)
...
function agregarRemeras() {
    ...
    nombre.innerHTML = carrito [00]
    cantidad.innerHTML =  carrito [01]
    talle.innerHTML = carrito [02]
    precioUnitario.innerHTML = "$"+ carrito [03]
    precioFinal.innerHTML = "$"+ carrito [04]
}